...
import { CheckCircle, Clock, Truck, Package, Home, ArrowLeft } from 'lucide-react';
import API from '../api';
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#09090b', color: '#fff', paddingTop: '100px', paddingBottom: '80px', paddingLeft: '5%', paddingRight: '5%' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
        <div>
          <h1 style={{ fontFamily: 'Outfit', fontSize: '28px', fontWeight: 800 }}>Order #{order._id.substring(0, 10).toUpperCase()}</h1>
          <p style={{ color: '#aaa', fontSize: '13px' }}>Placed on {new Date(order.createdAt).toLocaleDateString()}</p>
        </div>

        <span style={{
          background: order.orderStatus === 'Delivered' ? 'rgba(16, 185, 129, 0.2)' : 'rgba(212, 175, 55, 0.2)',
          color: order.orderStatus === 'Delivered' ? '#10b981' : '#d4af37',
          padding: '6px 16px',
          borderRadius: '20px',
          fontSize: '13px',
          fontWeight: 700
